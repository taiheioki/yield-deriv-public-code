...
The default values are 86400 (sec) for `--time` and 300 (K) for `--temperature`. The code load the file `./data/<DATA>` and saves the result to `./result/data/<DATA>_deriv.txt"`. Supported input file formats are `*_MinPATH.rrm` and `*_kINP.rrm`. The output file is formatted as:
